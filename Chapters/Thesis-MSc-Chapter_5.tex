...
CNNs have been widely recognized for their efficacy in exploiting local information, particularly in the context of computer vision tasks. These networks learn shared position-based kernels over a local window. The key advantage lies in their ability to capturing features such as edges and shapes effectively. Notably, CNNs have demonstrated success in various speech-related tasks, as exemplified by the Conformer architecture \cite{gulati2020conformer}.CNNs can effectively fuse channel-wise information within a local receptive field while learning task-specific details. This capability makes them adept at distilling knowledge encoded in pre-trained model parameters. Drawing inspiration from these properties, convolutional layers have found their way into the architecture of Adapter models. This integration allows Adapters to leverage the spatial processing capabilities of convolutional modules, further enhancing their ability to capture and adapt to intricate patterns within the data.
\subsection{BitFit}
